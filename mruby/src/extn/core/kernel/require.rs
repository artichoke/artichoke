...
pub struct Require {
    file: String,
    rust: Option<RequireFunc>,
    ruby: Option<Vec<u8>>,
}

impl Require {
    pub fn require(self, interp: Mrb) -> sys::mrb_value {
        ...
        if let Some(require) = self.rust {
            ...
            if response.is_err() {
                ...
                drop(context);
                drop(self);
                return LoadError::raise(interp, file.as_str());
            }
        }
...
pub fn method(interp: &Mrb, args: &Args) -> Result<Require, Error> {

use crate::extn::prelude::*;

pub fn init(interp: &mut Artichoke) -> InitializeResult<()> {
    
    
    let spec = crate::class::Spec::new("Addrinfo", None, None)?;
    interp.0.borrow_mut().def_class::<Addrinfo>(spec);
    
    
    
    let spec = crate::class::Spec::new("BasicSocket", None, None)?;
    interp.0.borrow_mut().def_class::<BasicSocket>(spec);
    
    
    
    let spec = crate::class::Spec::new("SocketError", None, None)?;
    interp.0.borrow_mut().def_class::<SocketError>(spec);
    
    
    
    let spec = crate::module::Spec::new(interp, "DRb", None)?;
    interp.0.borrow_mut().def_module::<DRb>(spec);
    
    
    
    let spec = crate::class::Spec::new("DRbIdConv", None, None)?;
    interp.0.borrow_mut().def_class::<DRbIdConv>(spec);
    
    
    
    let spec = crate::module::Spec::new(interp, "DRbUndumped", None)?;
    interp.0.borrow_mut().def_module::<DRbUndumped>(spec);
    
    
    
    let spec = crate::class::Spec::new("DRbObject", None, None)?;
    interp.0.borrow_mut().def_class::<DRbObject>(spec);
    
    
    
    let spec = crate::class::Spec::new("Socket", None, None)?;
    interp.0.borrow_mut().def_class::<Socket>(spec);
    
    
    
    let spec = crate::class::Spec::new("UDPSocket", None, None)?;
    interp.0.borrow_mut().def_class::<UDPSocket>(spec);
    
    
    
    let spec = crate::class::Spec::new("IPSocket", None, None)?;
    interp.0.borrow_mut().def_class::<IPSocket>(spec);
    
    
    
    let spec = crate::class::Spec::new("UNIXServer", None, None)?;
    interp.0.borrow_mut().def_class::<UNIXServer>(spec);
    
    
    
    let spec = crate::class::Spec::new("UNIXSocket", None, None)?;
    interp.0.borrow_mut().def_class::<UNIXSocket>(spec);
    
    
    
    let spec = crate::class::Spec::new("TCPServer", None, None)?;
    interp.0.borrow_mut().def_class::<TCPServer>(spec);
    
    
    
    let spec = crate::class::Spec::new("TCPSocket", None, None)?;
    interp.0.borrow_mut().def_class::<TCPSocket>(spec);
    
    
    
    interp.def_rb_source_file(
        b"drb.rb",
        &include_bytes!(concat!(env!("OUT_DIR"), "/src/generated/drb.rb"))[..]
    )?;
    
    interp.def_rb_source_file(
        b"drb/drb.rb",
        &include_bytes!(concat!(env!("OUT_DIR"), "/src/generated/drb/drb.rb"))[..]
    )?;
    
    interp.def_rb_source_file(
        b"drb/eq.rb",
        &include_bytes!(concat!(env!("OUT_DIR"), "/src/generated/drb/eq.rb"))[..]
    )?;
    
    interp.def_rb_source_file(
        b"drb/invokemethod.rb",
        &include_bytes!(concat!(env!("OUT_DIR"), "/src/generated/drb/invokemethod.rb"))[..]
    )?;
    
    Ok(())
}

#[derive(Debug)]
pub struct Addrinfo;


#[derive(Debug)]
pub struct BasicSocket;


#[derive(Debug)]
pub struct SocketError;


#[derive(Debug)]
pub struct DRb;


#[derive(Debug)]
pub struct DRbIdConv;


#[derive(Debug)]
pub struct DRbUndumped;


#[derive(Debug)]
pub struct DRbObject;


#[derive(Debug)]
pub struct Socket;


#[derive(Debug)]
pub struct UDPSocket;


#[derive(Debug)]
pub struct IPSocket;


#[derive(Debug)]
pub struct UNIXServer;


#[derive(Debug)]
pub struct UNIXSocket;


#[derive(Debug)]
pub struct TCPServer;


#[derive(Debug)]
pub struct TCPSocket;



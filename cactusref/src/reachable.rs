...
//! reaping used by [`Rc`](crate::Rc).
//!
//! This module provides automatic implementations for `&T`, `&mut T`,
//! `*const T`, `*mut T`, and `Box<T>` for any `T` that implements
//! [`Reachable`].

/// Reachability tests for objects wrapped by [`Rc`](crate::Rc). This trait is
/// unsafe.
///
/// **Warning**: If this trait is implemented incorrectly, [`Rc`](crate::Rc) has
/// undefined behavior. Memory may leak or be double freed.
pub unsafe trait Reachable {
    ...
    /// Reachability test used by [`Rc`](crate::Rc) to perform breadth first
    /// search for cycle detection in [`Drop`] implementation.
    ///
    /// **Warning**: If this function is implemented incorrectly,
    /// [`Rc`](crate::Rc) has undefined behavior. Memory may leak
    /// or be double freed.
